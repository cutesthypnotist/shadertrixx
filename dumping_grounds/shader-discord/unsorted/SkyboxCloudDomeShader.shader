	...
	Subshader
	{
		...
		Pass
		{
			...
			float4 drawImage(in float3 ro, in float3 rd )
			{
				...
				return lerp(float4(0.0,0.0,0.0,0.0), float4(1.0,1.0,1.0,1.0), fragColor * .2 + (1.0 - contr) * fragColor * fragColor * (3.0 - 2.0 * fragColor));
			}
