...
// Quick skybox mod by Merlin

Shader "Skybox/Procedural Night Skybox" {
Properties {
	...
	_NightSkyboxBrightness("Night Skybox Brightness", Float) = 1.0
	_NightSkyboxExtinctionFactor("Night Skybox Extinction Factor", Float) = 20.0

	_SkyboxDirection("Skybox Direction", Vector) = (0, 0, 0, 0)
}
...
SubShader {
    ...
    Pass {
		...
		uniform float _NightSkyboxBrightness;
		uniform float _NightSkyboxExtinctionFactor;
		uniform float4 _SkyboxDirection;
        ...
        //    half3   rayDir          : TEXCOORD0;
			...
			nointerpolation half3x3 skyboxLookAt	: TEXCOORD4;
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
        ...
        //    OUT.rayDir          = half3(-eyeRay);
			...
			sincos(_SkyboxDirection.x, xSin, xCos);
			sincos(_SkyboxDirection.y, ySin, yCos);
			sincos(_SkyboxDirection.z, zSin, zCos);
			...
			OUT.skyboxLookAt = mul(xRot, mul(yRot, zRot));
        ...
        //    half y = ray.y / SKY_GROUND_THRESHOLD;
			...
			// Sample night skybox
		#if SKYBOX_SUNDISK == SKYBOX_SUNDISK_HQ || SKYBOX_SUNDISK == SKYBOX_SUNDISK_SIMPLE
			float luma = dot(col.rgb, lumaConv);
			float3 skyboxCol = texCUBElod(_NightSkybox, float4(mul(IN.skyboxLookAt, ray.xyz), 0));
			col += (skyboxCol / max(1, luma * _Exposure * _NightSkyboxExtinctionFactor)) * _NightSkyboxBrightness;
		#endif
